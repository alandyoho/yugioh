        ...
        <React.Fragment>
            <View style={{ flex: 1, flexDirection: "row" }}>
                ...
                <TouchableOpacity style={{ flex: 1, width: 50, height: null, borderColor: "rgb(130, 69, 91)", borderRadius: 10, borderWidth: 2 }}>
                    {boardsRetrieved == true && opponentBoard.banishedZone.length > 0 && <FadeScaleImage source={{ uri: opponentBoard.banishedZone[opponentBoard.banishedZone.length - 1].card_images[0].image_url_small }} resizeMode={"contain"} style={{ flex: 1, width: null, height: null }} />}
                </TouchableOpacity>
            </View>
            ...
            <View style={{ flex: 1, flexDirection: "row" }}>
                ...
                {[1, 2, 3, 4, 5].map(cardIndex => (
                    <TouchableOpacity key={cardIndex} style={{ flex: 1, width: 50, height: null, borderColor: "black", borderRadius: 10, borderWidth: 2 }} onPress={(boardsRetrieved == true && opponentBoard.m1[cardIndex]['card'].exists && !opponentBoard.m1[cardIndex]['card'].set) ? () => examineOpponentCard(opponentBoard.m1[cardIndex]['card']) : () => console.log("no card in slot")}>
                        {boardsRetrieved == true && opponentBoard.m1[cardIndex]['card'].exists && (opponentBoard.m1[cardIndex]['card'].set ? <FadeScaleImage source={require("../../assets/default_card.png")} resizeMode={"contain"} style={{ flex: 1, width: null, height: null, transform: [{ rotate: '90deg' }] }} /> : (opponentBoard.m1[cardIndex]['card'].defensePosition ? <FadeScaleImage source={{ uri: opponentBoard.m1[cardIndex]['card'].card_images[0].image_url_small }} resizeMode={"contain"} style={{ flex: 1, width: null, height: null, transform: [{ rotate: '90deg' }] }} /> : <FadeScaleImage source={{ uri: opponentBoard.m1[cardIndex]['card'].card_images[0].image_url_small }} resizeMode={"contain"} style={{ flex: 1, width: null, height: null }} />))}
                    </TouchableOpacity>
                ))}
                <TouchableOpacity style={{ flex: 1, width: 50, height: null, borderColor: 'rgb(130, 69, 91)', borderRadius: 10, borderWidth: 2 }} onPress={() => toggleOpponentGraveyardPopup("request")}>
                    {boardsRetrieved == true && opponentBoard.graveyard.length > 0 && <FadeScaleImage source={{ uri: opponentBoard.graveyard[opponentBoard.graveyard.length - 1].card_images[0].image_url_small }} resizeMode={"contain"} style={{ flex: 1, width: null, height: null }} />}
                </TouchableOpacity>
            </View>
            ...
            <View style={{ flex: 1, flexDirection: "row" }}>
                ...
                {[1, 2, 3, 4, 5].map(cardIndex => (
                    <TouchableOpacity key={cardIndex} style={{ flex: 1, width: 50, height: null, borderColor: 'black', borderRadius: 10, borderWidth: 2 }} onPress={(boardsRetrieved == true && opponentBoard.st[cardIndex]['card'].exists && !opponentBoard.st[cardIndex]['card'].set) ? () => examineOpponentCard(opponentBoard.st[cardIndex]['card']) : () => console.log("no card in slot")}>
                        {boardsRetrieved == true && opponentBoard.st[cardIndex]["card"].exists && (opponentBoard.st[cardIndex]['card'].set ? <FadeScaleImage source={require("../../assets/default_card.png")} resizeMode={"contain"} style={{ flex: 1, width: null, height: null }} /> : <FadeScaleImage source={{ uri: opponentBoard.st[cardIndex]['card'].card_images[0].image_url_small }} resizeMode={"contain"} style={{ flex: 1, width: null, height: null }} />)}
                    </TouchableOpacity>
                ))}
                <TouchableOpacity style={{ flex: 1, width: 50, height: null, borderColor: 'rgb(130, 69, 91)', borderRadius: 10, borderWidth: 2 }} >
                    <ImageBackground source={require("../../assets/default_card.png")} resizeMode={"contain"} style={{ width: "100%", height: "100%" }} >
