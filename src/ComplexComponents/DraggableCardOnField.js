    ...
    render() {
        return (
            <Animated.View
                ...
                <TouchableOpacity disabled={this.props.user ? true : false} onLongPress={this.examineCard} onPress={this.handleDoubleTap} onPressOut={this.props.dismissExaminePopup} activeOpacity={1} style={{ flexDirection: "column", justifyContent: "center", alignItems: "center", transform: this.props.user ? (this.props.item.user && this.props.user === this.props.item.user ? [{ rotate: "0deg" }] : [{ rotate: '180deg' }]) : [{ rotate: '180deg' }] }}>
                    <FadeScaleImage source={this.props.item.set ? require("../../assets/default_card.png") : { uri: this.props.item.card_images[0].image_url_small }} resizeMode={"contain"} style={[{ transform: this.props.user ? (this.props.user === this.props.host ? [{ rotate: '180deg' }] : [{ rotate: "0deg" }]) : (this.props.item.defensePosition ? [{ rotate: '270deg' }] : [{ rotate: '180deg' }]) }, this.state.overReturnToHand ? { width: 150 * 0.65, height: 300 * 0.65 } : { flex: 1, width: 50, height: 100 }]} />
                </TouchableOpacity>
            </Animated.View>
        );
    }
